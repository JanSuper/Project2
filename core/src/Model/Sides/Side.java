...
public abstract class Side implements Collide{
   Vector2 [] vertices = new Vector2[2];
    ...
    public boolean collideIfCollision(Solver solver) {
        ...
        int currentPosSide = Intersector.pointLineSide(vertices[0], vertices[1], new Vector2((float) solver.getPosition().getX() +(float) Math.signum(solver.getVelocity().getX()), (float) solver.getPosition().getY()+ (float)Math.signum(solver.getVelocity().getY())));
